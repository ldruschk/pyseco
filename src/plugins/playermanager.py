...
class playermanager(pyseco_plugin):
    join_msg = "$fff$i>> %sPlayer $z$s%s$z$s%s$i has joined the game."
    leave_msg = "$s$fff$i>> %sPlayer $z$s%s$z$s%s$i has left the game."
    ...
    def process_callback(self, value):
        if value[1] == "TrackMania.PlayerConnect":
            ...
            player_info = self.pyseco.query((value[0][0]["Login"], ),
                                            "GetPlayerInfo")
    ...
    def player_add(self, player_info):
        if player_info["Login"] not in self.pyseco.players:
            ...
            self.pyseco.send_chat_message(self.join_msg %
    ...
    def player_remove(self, player_login):
        if player_login in self.pyseco.players:
            ...
            self.pyseco.send_chat_message(self.leave_msg %
                                          (self.pyseco.chat_color,
                                           player.get_nick_name(),
                                           self.pyseco.chat_color))
